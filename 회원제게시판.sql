...
-- 사용자 계정 생성
CREATE USER member_sde IDENTIFIED BY member1234;

-- 생성한 사용자 계정에 권한 부여
GRANT CONNECT, RESOURCE, CREATE VIEW TO member_sde;

-- 테이블 스페이스 할당
ALTER USER member_Sde DEFAULT 
...
CREATE TABLE MEMBER(
	...
	MEMBER_NM VARCHAR2(30) NOT NULL, -- NAME
	MEMBER_GENDER CHAR(1) CHECK(MEMBER_GENDER IN ('M', 'F')),
	ENROLL_DATE DATE DEFAULT SYSDATE,
	SECESSION_FL CHAR(1) DEFAULT 'N' CHECK(SECESSION_FL IN ('Y', 'N')) --FLAG
);
...
CREATE TABLE "BOARD"(
	...
	CREATE_DT DATE DEFAULT SYSDATE, -- DATE를 컬럼명에는 보통 DT로 적음
	READ_COUNT NUMBER DEFAULT 0,
	DELETE_FL CHAR(1) DEFAULT 'N' CHECK (DELETE_FL IN ('N', 'Y')),
	MEMBER_NO NUMBER CONSTRAINT BOARD_WIRTER_FK REFERENCES "MEMBER"
);
...
VALUES (SEQ_BOARD_NO.NEXTVAL, '샘플 제목 1', '샘플 내용 1 입니다.',
		TO_DATE('2022-09-20 10:30:29', 'YYYY-MM-DD HH24:MI:SS'),
		DEFAULT, DEFAULT, 3);	

INSERT INTO BOARD 
VALUES (SEQ_BOARD_NO.NEXTVAL, '샘플 제목 2', '샘플 내용 2 입니다.',
		TO_DATE('2022-09-21 20:18:29', 'YYYY-MM-DD HH24:MI:SS'),
		DEFAULT, DEFAULT, 3);	
...
COMMENT ON COLUMN "COMMENT".BOARD_NO 		IS '댓글 회원번호';
...
DELETE FROM "COMMENT" WHERE BOARD_NO = 3;
COMMIT;
...
--
	CASE 
		WHEN SYSDATE - CREATE_DT < 1/24/60 -- 1분 미만인 경우
		THEN FLOOR((SYSDATE - CREATE_DT)*24*60*60) || '초 전'
--		
		WHEN SYSDATE - CREATE_DT < 1/24 -- 1시간 미만인 경우
		THEN FLOOR((SYSDATE - CREATE_DT)*24*60) || '분 전'
--		
		WHEN SYSDATE - CREATE_DT < 1 -- 1일 미만인 경우
...
SELECT BOARD_NO, BOARD_TITLE, BOARD_CONTENT,
		...
		TO_CHAR(CREATE_DT, 'YYYY-DD-MM HH24:MI:SS') CREATE_DT
FROM "BOARD"
JOIN "MEMBER" USING (MEMBER_NO)
WHERE DELETE_FL = 'N'
AND BOARD_NO = 1;
...
-- 상세 조회된 게시글의 조회 수 증가
UPDATE "BOARD" SET 
READ_COUNT = READ_COUNT + 1 -- UPDATE시만 대입연산자로 사용(원래는 같다라는 의미)
